...
# Load the dataset
train_file_path = 'D:/hehai/textCNN/公众训练.xlsx'  # Update this path
train_df = pd.read_excel(train_file_path)

# Extract text and labels
X = train_df['原创微博'].values
y = train_df[['Problem solving', 'Instrumental social support offering',
...
checkpoint = ModelCheckpoint('textcnn_model_best.keras', monitor='val_accuracy', save_best_only=True, mode='max')

# Train the model
model.fit(X_train, y_train, epochs=5, batch_size=32, validation_data=(X_test, y_test), callbacks=[checkpoint])

# Save the final model
model.save('textcnn_model_final.keras')
