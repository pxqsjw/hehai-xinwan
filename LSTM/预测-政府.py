...
# 加载保存的 LSTM 模型和分词器
model = load_model('lstm_model_best_government.keras')

with open('tokenizer.pkl', 'rb') as f:
    tokenizer = pickle.load(f)

# 加载测试数据集
test_file_path = 'D:/hehai/LSTM/政府测试.xlsx'  # 更新路径
...
# 计算一致性评分（正确预测的标签数量）
correct_predictions = np.sum(predicted_labels == test_df[['Command and coordination', 'Personnel rescue and relocation', 'Facility defense and repairs',
        'Information release and crisis communication', 'Recovery of economy and life']].values, axis=1)
...
classification_rep = classification_report(y_true, predicted_labels, target_names=[
    'Command and coordination', 'Personnel rescue and relocation', 'Facility defense and repairs',
        'Information release and crisis communication', 'Recovery of economy and life'], output_dict=False)
